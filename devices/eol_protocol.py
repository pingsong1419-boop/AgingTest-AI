...
class EOLProtocol:
    ...
    def transact(self, device_id: int, operation: int, payload: Optional[bytes] = None,
        ...
        request_data = bytes([self.REQUEST_PREFIX, device_id & 0xFF, operation & 0xFF]) + payload
        request_data = request_data[:8].ljust(8, b"\x00")
    ...
    def _build_operations(self) -> Dict[str, Dict[str, Any]]:
        return {
            "insulation_control": {"device_id": 0x03, "operation": 0x01, "payload": lambda kw: bytes([0x00, self._int_arg(kw, "STATE", "VALUE")])},
            ...
            "read_pwm_duty": {"device_id": 0x05, "operation": 0x01, "payload": lambda kw: bytes([self._int_arg(kw, "PWM", "CHANNEL", "INDEX")]), "decoder": lambda raw: raw[4] if len(raw) > 4 else None},
            "read_pwm_freq": {"device_id": 0x05, "operation": 0x02, "payload": lambda kw: bytes([self._int_arg(kw, "PWM", "CHANNEL", "INDEX")]), "decoder": self._decode_u32_from_4},
            ...
            "read_csc_hv": {"device_id": 0x07, "operation": 0x02, "decoder": lambda raw: self._decode_u32_from_4(raw) * 0.001},
            ...
            "read_stack_voltage": {"device_id": 0x07, "operation": 0x05, "decoder": lambda raw: self._decode_u32_from_4(raw) * 0.001},
            "read_fast_charge_impedance": {"device_id": 0x07, "operation": 0x06, "decoder": self._decode_u32_from_4},
            "read_crash_pwm_duty": {"device_id": 0x08, "operation": 0x01, "decoder": lambda raw: raw[4] if len(raw) > 4 else None},
            "read_crash_pwm_freq": {"device_id": 0x08, "operation": 0x02, "decoder": self._decode_u32_from_4},
            "read_crash_impedance": {"device_id": 0x08, "operation": 0x03, "decoder": self._decode_u32_from_4},
            "read_crash_pulse_width": {"device_id": 0x08, "operation": 0x04, "decoder": self._decode_u32_from_4},
            ...
            "read_pressure_sensor": {"device_id": 0xFF, "operation": 0x02, "decoder": self._decode_u32_from_4},
            "read_hsd_load_voltage": {"device_id": 0xFF, "operation": 0x03, "decoder": self._decode_u32_from_4},
        }
    ...
    def _decode_payload_hex(self, raw: bytes):
        return raw[4:].hex(" ").upper() if len(raw) > 4 else ""
    ...
    def _decode_u32_from_4(self, raw: bytes):
        data = raw[4:8]
    ...
    def _decode_current(self, raw: bytes):
        value = self._decode_u32_from_4(raw)
